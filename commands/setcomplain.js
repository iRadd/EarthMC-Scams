module.exports = {
	name: 'setcomplain',
	description: 'mod/admin command to set a user\'s complain count',
    ...
    if (!member.roles.cache.some(role => role.name === 'Moderator')) {
      message.channel.send({ content: `> You do not have permission to execute the following command, \`.setcomplain\``});
    ...
    if(!args[0]) {
        message.channel.send({ content: `> You need to mention a proper username to set their complain number\n> EG: \`.setcomplain iRadioo 0\` or \`.setcomplain Fix 87\``})
        message.reactions.removeAll().catch(error => console.error('Failed to clear reactions: ', error));
        message.react('863121138947063818');
        return
      }
    ...
    if(!args[1]) {
      message.channel.send({ content: `> You need to mention a proper number to set ${args[0]}'s Complain Count\n> EG: \`.setcomplain iRadioo 0\` or \`.setcomplain Fix 87\``})
      message.reactions.removeAll().catch(error => console.error('Failed to clear reactions: ', error));
      message.react('863121138947063818');
      return
    }
        ...
        if (player == undefined) {
          ...
          var mcuser = new db.table('mcuser')
            ...
            if (complaints.get(`${player.uuid}`) == null) {
              ...
              complaints.set(`${player.uuid}`, args[1])

              message.channel.send({ content: `> I've set **${player.username}** \`${player.uuid}\`'s Complaint Count to \`${args[1]}\``})
              message.reactions.removeAll().catch(error => console.error('Failed to clear reactions: ', error));
              message.react('875428843454865439');
